    ...
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <div className={`border rounded-xl p-4 flex flex-col justify-between shadow-sm transition-all duration-300 ${
        ...
        <div className="flex items-baseline justify-between mt-2">
          ...
          <span className={`text-[10px] font-semibold ${stats.redFlagCount > 0 ? 'text-red-700' : 'text-text-secondary'}`}>
            {stats.redFlagCount > 0 ? 'NEEDS CLINICAL CALL' : 'no unconfirmed cases'}
          </span>
        </div>
      </div>
