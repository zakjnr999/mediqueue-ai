...
| `fullName` | String | Patient's full name (stored without field-level encryption; stripped from the AI payload during triage). |
| `phoneNumber` | String | Patient's phone number (stored without field-level encryption; stripped from the AI payload during triage). |
